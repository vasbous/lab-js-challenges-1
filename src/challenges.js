...
function multiplyBy(listNumbers, multiplier) {
  let result = [];
...
multiplyBy(numbers, 5);

// Iteration 4 | Filter Out
const original = ["cat", "dog", "fish", "bird", "cat", "fish"];
const toRemove = ["cat", "dog"];

function filterOut(origString, arrRemove) {
  if (origString.length === 0) {
    return null;
  } else if (arrRemove.length === 0) {
    return origString;
  } else {
    let filteredArray = origString.filter((item) => !arrRemove.includes(item));
...
function uniquifyArray(wordList) {
  ...
  for (let i = 0; i < wordList.length; i++) {
    if (uniqueArray.includes(wordList[i]) === false) {
